...
[@fig:imd_oomd] shows the IMD-OOMD plot given this model and the theoretical limits calculated with it. 

![PCA-modelled data points on the IMD-OOMD plane with 0.95 confidence interval. ](./plots/HT2_DModX_example_plot.png){#fig:imd_oomd}
...
Summary statistics of PLS models can be displayed in similar plots to [@fig:imd_oomd].
...
The latter serves as a measure for what input features the model considers predictive of the output, see [@fig:reg_vect].

![Visualization of a PLS regression vector, generated from simulated test data inspired by spectroscopy.](./plots/regression_vector_plot.png){#fig:reg_vect}
...
![Computational complexity as measured by runtime (upper row), peak memory consumption (middle row), and accuracy in reconstructing the input data measured by mean residual (lower row). Scaling of quantites evaluated with respect to `n_components` (left columns), dimensionality of the dataset (number of features, middle column), and number of samples (right column).](./plots/combined_complexity.png){#fig:comb_comp}

Figure [@fig:comb_comp] compares the performance of `open_nipals.NipalsPCA`, measured by runtime, memory requirements, and accuracy, for varying `n_components`, dimensionalities, and number of samples, to alternative dimensionality reduction and imputation techniques.
...
Therefore, this datapoint was omitted in the middle plot of the middle row in [@fig:comb_comp].

## Computational complexity and accuracy vs. number of latent variables
The leftmost column of [@fig:comb_comp] shows complexity and accuracy evaluated over a range of latent variables `n_components = 2 - 16`.

The upper left plot of [@fig:comb_comp] shows the runtime cost of `open_nipals.NipalsPCA` increases with `n_components` at less-than exponential scaling, with ICA being by far the slowest option for `n_components>4`, FA/EM being the slowest option at `n_components<=4`, and `sklearn.PCA` the fastest option globally.
`sklearn.PCA` relies on SVD, therefore the numerical cost (runtime and memory) of inferring any number of latent variables is equal, which can be seen from the upper left and the middle left panel of [@fig:comb_comp]. 
...
The peak memory consumption vs. `n_components` is plotted in the left plot of the middle row of [@fig:comb_comp]. 
...
The middle column of [@fig:comb_comp] shows the scaling of computational complexity and accuracy with the dimensionality of the data space (`n_features = 20 - 1000`).
...
The upper middle panel of [@fig:comb_comp] shows that `open_nipals.NipalsPCA`' model fitting has a high but competitive runtime, surpassed by FA/EM at small and ICA at 100 features, while being the slowest option at 1000 features. Its runtime in seconds vs. dimensionality curve is quite shallow (5e-2 - 2e-1), even compared to `sklearn.PCA` (5e-3 - 7e-2), which was faster than its competitors by 1-2 orders of magnitude throughout the investigated range of features.
...
The memory consumption in MB vs. features is plotted in the middle row's middle plot of [@fig:comb_comp]. 
...
The accuracy is evaluated against different number of features in the lower middle panel of [@fig:comb_comp].
...
The rightmost column of [@fig:comb_comp] displays computational complexity and accuracy over a broad range of dataset size (`n_samples = 50 - 10000`).

The runtime requirement scaling with `n_samples` can be found in the upper right plot of [@fig:comb_comp]. 
...
As can be seen from the middle right panel of [@fig:comb_comp], the peak memory consumption of `open_nipals.NipalsPCA` follows exponential scaling with `n_samples` ([@fig:comb_comp]), which is competitive with its alternatives.
...
The numerical accuracy as a function of `n_samples` in [@fig:comb_comp] remains quite constant, with `open_nipals.NipalsPCA` being slightly more accurate than all of its competitors throughout the entire range.
...
A jupyter [notebook](placeholder for git link) to reproduce [@fig:imd_oomd], another [notebook](placeholder for git link) to generate [@fig:reg_vect], and a [notebook](placeholder for git link) to generate [@fig:comb_comp] can be found in [@on_github] under the `paper` branch.
